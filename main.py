...
def load_archived_students():
    try:
        rows = get_meta_sheet().get_all_values()
        for row in rows:
            if len(row) >= 2 and row[0] == "archived":
                _archived_students.add(row[1].strip())
    except Exception as e:
        print(f"load_archived_students ошибка: {e}")

def archive_student(name: str):
    _archived_students.add(name)
    get_meta_sheet().append_row(["archived", name])

def restore_student(name: str):
    _archived_students.discard(name)
    ws = get_meta_sheet()
    rows = ws.get_all_values()
    for i, row in enumerate(rows):
        if len(row) >= 2 and row[0] == "archived" and row[1].strip() == name:
            ws.delete_rows(i + 1)
            break
...
def kb_trainer_students():
    ...
    b.adjust(2)
    arc_count = len(_archived_students)
    b.button(text="➕ Добавить ученика", callback_data="tr_add_student")
    b.button(text=f"📦 Архив ({arc_count})" if arc_count else "📦 Архив", callback_data="tr_archive")
    b.button(text="◀️ Назад",            callback_data="tr_menu")
    b.adjust(2, 1, 1, 1)
    return b.as_markup()
...
def kb_trainer_student(name: str):
    ...
    b.button(text="📦 В архив",         callback_data=f"tr_to_archive_{name}")
    b.button(text="◀️ Назад",           callback_data="tr_students")
    b.adjust(2, 2, 1, 1, 1)
    return b.as_markup()

def kb_trainer_archive():
    b = InlineKeyboardBuilder()
    for name in _archived_students:
        grade = _known_grades.get(name, "")
        label = f"{name}  {grade}".strip() if grade else name
        b.button(text=label, callback_data=f"tr_archived_{name}")
    b.adjust(2)
    b.button(text="◀️ Назад", callback_data="tr_students")
    if _archived_students:
        b.adjust(2, 1)
    return b.as_markup()
...
# ── Архив учеников ──────────────────────────────────────────────
@dp.callback_query(F.data == "tr_archive")
async def cb_tr_archive(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    count = len(_archived_students)
    text = f"📦 *Архив учеников*\n\n{'Выберите ученика:' if count else 'Архив пуст'}"
    await callback.message.edit_caption(caption=text, reply_markup=kb_trainer_archive(), parse_mode="Markdown")
    try: await callback.answer()
    except: pass

@dp.callback_query(F.data.startswith("tr_to_archive_"))
async def cb_tr_to_archive(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    name = callback.data[len("tr_to_archive_"):]
    b = InlineKeyboardBuilder()
    b.button(text="✅ Подтвердить", callback_data=f"tr_archive_yes_{name}")
    b.button(text="❌ Отменить",    callback_data=f"tr_student_{name}")
    b.adjust(2)
    await callback.message.edit_caption(
        caption=f"📦 Перевести *{name}* в архив?\n\nУченик будет скрыт из основного списка.",
        reply_markup=b.as_markup(), parse_mode="Markdown")
    try: await callback.answer()
    except: pass

@dp.callback_query(F.data.startswith("tr_archive_yes_"))
async def cb_tr_archive_yes(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    name = callback.data[len("tr_archive_yes_"):]
    archive_student(name)
    await callback.message.edit_caption(
        caption=f"✅ *{name}* перемещён в архив.",
        reply_markup=kb_trainer_students(), parse_mode="Markdown")
    try: await callback.answer()
    except: pass

@dp.callback_query(F.data.startswith("tr_restore_yes_"))
async def cb_tr_restore_yes(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    name = callback.data[len("tr_restore_yes_"):]
    restore_student(name)
    await callback.message.edit_caption(
        caption=f"✅ *{name}* возвращён из архива.",
        reply_markup=kb_trainer_students(), parse_mode="Markdown")
    try: await callback.answer()
    except: pass

@dp.callback_query(F.data.startswith("tr_restore_"))
async def cb_tr_restore(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    name = callback.data[len("tr_restore_"):]
    b = InlineKeyboardBuilder()
    b.button(text="✅ Подтвердить", callback_data=f"tr_restore_yes_{name}")
    b.button(text="❌ Отменить",    callback_data=f"tr_archived_{name}")
    b.adjust(2)
    await callback.message.edit_caption(
        caption=f"↩️ Вернуть *{name}* из архива?",
        reply_markup=b.as_markup(), parse_mode="Markdown")
    try: await callback.answer()
    except: pass

@dp.callback_query(F.data.startswith("tr_archived_"))
async def cb_tr_archived_student(callback: CallbackQuery):
    if not is_trainer(callback.from_user.id): return
    name = callback.data[len("tr_archived_"):]
    if name not in USER_COLUMNS:
        try: await callback.answer("Ученик не найден")
        except: pass
        return
